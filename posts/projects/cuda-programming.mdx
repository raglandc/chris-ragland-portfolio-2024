...
<Image
    title="Load coordinates into registers"
    link="/img/post-images/project-images/cuda/picture-1.webp"
/>
...
<Image
    title="Set up shared memory"
    link="/img/post-images/project-images/cuda/picture-2.webp"
/>
We set every value in the private histogram to be zero.

<Image
    title="Zero out shared memory"
    link="/img/post-images/project-images/cuda/picture-3.webp"
/>
...
<Image
    title="Calculating distance for shared memory"
    link="/img/post-images/project-images/cuda/picture-4.webp"
/>
After every R’th block has been loaded and the distances have been calculated,
the only distances left to calculate are those within itself. To do this, every
block places it’s associated value from L into shared memory for all other
threads of the same block to access.

<Image
    title="Place L into shared memory"
    link="/img/post-images/project-images/cuda/picture-5.webp"
/>
...
<Image
    title="Intra-block calculations"
    link="/img/post-images/project-images/cuda/picture-6.webp"
/>
Finally, every block will take the data it has collected over the course of the
previous steps and update global memory with the private value. This can be done
without atomic instructions because instead of having every block competing to
fill in the same buckets for a single histogram in global memory, we have the
blocks update private copies in global memory. This will allow us to write a
reduction kernel that can perform a sum reduction for every bucket in every
block.

<Image
    title="Build private histograms"
    link="/img/post-images/project-images/cuda/picture-7.webp"
/>
...
<Image
    title="Dynamic shared memory"
    link="/img/post-images/project-images/cuda/picture-8.webp"
/>

The first step is to load every bucket from that column into shared memory for the block.

<Image
    title="Load buckets into shared memory"
    link="/img/post-images/project-images/cuda/picture-9.webp"
/>
Since our thread count is designed to have one thread per row (block) of the
column, we have to in a way pad our shared memory so that every value is counted
in the striding. We get the closest power of 2 for each block that will
determine our stride.

<Image
    title="Pad the shared memory incase block size is not a power of two"
    link="/img/post-images/project-images/cuda/picture-10.webp"
/>

Then we actually perform the stride on shared memory.

<Image
    title="Perform stride"
    link="/img/post-images/project-images/cuda/picture-11.webp"
/>
...
<Image
    ...
    link="/img/post-images/project-images/cuda/picture-12.webp"
/>
...
<Image
    title="Results before optimizations"
    link="/img/post-images/project-images/cuda/picture-13.webp"
/>
...
<Image
    title="Results after optimizations"
    link="/img/post-images/project-images/cuda/picture-14.webp"
/>
